...
WAIT_TIME = 100  # time to wait for each line to draw


class GUI:
    """Class for GUI
    """
    def __init__(self, size: list, pg: pygame, tiles: Tile) -> None:
        self._tiles = tiles
        self._size = size
        self._rect = pg.Rect
    ...
    def draw_path(self, pos_chain: list, target_pos: dict, board_size: int):
        """Draw single or mutiple line

        Args:
            pos_chain (list): posistions to draw
            target_pos (dict): position to move
        ...
        for column in range(board_size):
            for row in range(board_size):
                ...
                self._draw.rect(
                    ...
                    color,
                    self._rect(
                        rect_width * column, rect_width * row, rect_width, rect_width
                    ),
                )
